...
class Merchant(models.Model):
    ...
    user                        = models.ForeignKey(User, null=True, on_delete=models.SET_NULL, help_text="Marchant's owner (User FK).")
    category                    = models.CharField(max_length=20, choices=MERCHANT_CATEGORY_CHOICES)
    parent                      = models.ForeignKey("self", blank=True, null=True, on_delete=models.SET_NULL, help_text="If this marchant is under a company and has superior/parent marchant/company then this is the reference of that parent/company marchant. Otherwise it's not applicable.")
    business_address            = models.ForeignKey(Address, related_name='merchant_business_address', null=True, on_delete=models.SET_NULL, help_text="Business address (Address FK).")
    pickup_address              = models.ForeignKey(Address, related_name='merchant_pickup_address', null=True, on_delete=models.SET_NULL, help_text="Pickup address (Address FK).")
    ...
    total_merchant_parcel_trx   = models.DecimalField(default=0.00, max_digits=30, decimal_places=2, help_text="Total transaction amount between Marchant and Parcel. [Automatic Process]")
    total_merchant_admin_trx    = models.DecimalField(default=0.00, max_digits=30, decimal_places=2, help_text="Total transaction amount between Marchant and Admin. [Automatic Process]")
